package tgb.cryptoexchange.billing.controller;
...
public class TransactionGrpcService extends TransactionsServiceGrpc.TransactionsServiceImplBase {
    ...
    @Override
    public void getTransactions(GetTransactionsGrpc request, StreamObserver<GetTransactionsResponseGrpc> responseObserver) {
        PaginationParams pagination = request.getPagination();

        Page<TransactionDTO> dtoPage = transactionService.findTransactions(transactionMapper.buildFindSpecification(request),
                pagination.getPage(),
                pagination.getSize(),
                pagination.getSortersList().stream().toList());

        GetTransactionsResponseGrpc response = GetTransactionsResponseGrpc.newBuilder()
                .addAllTransactions(dtoPage.getContent().stream().map(transactionMapper::toTransactionResponse).toList())
