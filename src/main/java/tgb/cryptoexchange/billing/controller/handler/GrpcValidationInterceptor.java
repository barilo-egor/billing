        ...
        private void handleValidationViolations(ValidationResult result) {
            ...
            for (Violation violation : result.toProto().getViolationsList()) {
                ...
                if (violation.hasField() && violation.getField().getElementsCount() > 0) {
                    fieldName = violation.getField().getElements(0).getFieldName();
                } else if (violation.hasField()) {
                    fieldName = violation.getField().toString();
                }
                ...
                badRequestBuilder.addFieldViolations(
                        BadRequest.FieldViolation.newBuilder()
                                .setField(fieldName)
                                .setDescription("Should not be empty.")
                                .build()
                );
            }
